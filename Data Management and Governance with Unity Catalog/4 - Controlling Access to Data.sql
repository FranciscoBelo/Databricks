...
-- MAGIC
-- MAGIC - Navigate to the top-right of this notebook and click the drop-down menu to select your cluster. By default, the notebook will use **Serverless**.
...
-- MAGIC %md
-- MAGIC ## Task 1: Controlling Access to Data
-- MAGIC
...
-- MAGIC %md
-- MAGIC ### 1.1 Generate an SQL Query to Access the Table
-- MAGIC
...
-- MAGIC
-- MAGIC Example: `SELECT * FROM someone_elses_catalog.schema.view;`
...
-- MAGIC %md
-- MAGIC ### 1.2 Inherited Privileges
-- MAGIC
...
-- MAGIC %md
-- MAGIC ### 1.4 Explicit Privileges
-- MAGIC
-- MAGIC Using explicit privilege grants, let's build a permission chain that will allow anyone to access the *gold* view.
-- MAGIC
-- MAGIC **NOTE:** For user's to access a schema within a catalog you will also have to grant `USE CATALOG` on the catalog. This will not work in this shared training environment. You do not have permission to share your catalog.
...
-- MAGIC %md
-- MAGIC ## Task 2: Row and Column Security
...
-- MAGIC %md
-- MAGIC #### 2.1.2 Create a Function to Perform Column Masking
-- MAGIC
...
-- MAGIC %md
-- MAGIC #### 2.1.3 Alter the Table to Apply the Mask
...
-- MAGIC %md
-- MAGIC #### 2.1.4 Query the Table with Masking
...
-- MAGIC %md
-- MAGIC #### 2.1.5 Alter the Table to Drop the Mask
...
-- MAGIC %md
-- MAGIC #### 2.1.6 Query the Table after Removing the Mask
...
-- MAGIC %md
-- MAGIC #### 2.1.7 Drop the Mask Function
...
-- MAGIC %md
-- MAGIC ### 2.2 Row Filtering
-- MAGIC Let us implement row filtering on the **silver** table and analyze it.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC #### 2.2.1 Query the Table before Row Filtering
...
-- MAGIC %md
-- MAGIC #### 2.2.2 Create a Function to Perform Row Filtering
-- MAGIC
...
-- MAGIC %md
-- MAGIC #### 2.2.3 Alter the Table to Apply the Row Filter
...
-- MAGIC %md
-- MAGIC #### 2.2.4 Query the Table with Row Filtering
...
-- MAGIC %md
-- MAGIC #### 2.2.5 Alter the Table to Drop the Row Filter
...
-- MAGIC %md
-- MAGIC #### 2.2.6 Query the Table after Removing the Row Filter
...
-- MAGIC %md
-- MAGIC #### 2.2.7 Drop the Row Filter Function
...
-- MAGIC %md
-- MAGIC #### 3.1.2 Re-issue Grant Access to View
...
-- MAGIC %md
-- MAGIC #### 3.2.2 Re-issue Grant Access to View
-- MAGIC We'll re-issue the grant since the above statement replaced the previous object and thus any grants applied directly on the object would have been lost.
