...
-- MAGIC %md
-- MAGIC In this course, we may use dynamically generated variables created by the classroom setup scripts to reference your catalog and/or schema within the `DA` object. For example, the `DA.catalog_name` variable will dynamically reference your specific catalog when executing SQL code.
-- MAGIC
-- MAGIC
...
-- MAGIC    - Select the catalog icon on the left.
-- MAGIC
-- MAGIC    - Expand your unique catalog name.
-- MAGIC
-- MAGIC    - Expand the **example** schema we previously created.
-- MAGIC
-- MAGIC    - Leave your catalog explorer open on the left.
...
-- MAGIC
-- MAGIC    - Select the **Catalog** icon (3rd one in icon list) in the left vertical bar (directly under **File** menu).
-- MAGIC
-- MAGIC    - Expand your unique catalog name.
-- MAGIC
-- MAGIC    - Expand the **example** schema we previously created. Notice that the schema now contains **Tables** and **Functions**.
-- MAGIC
-- MAGIC    - Expand **Functions** (you might have to refresh your schema).
-- MAGIC    
-- MAGIC    -  Notice that the newly created function, **dbacademy_mask**, is available in your schema.
-- MAGIC
-- MAGIC
-- MAGIC
