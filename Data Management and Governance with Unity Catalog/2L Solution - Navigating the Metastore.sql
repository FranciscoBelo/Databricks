...
-- MAGIC
-- MAGIC Additionally, we'll examine the roles of system catalogs and information_schema in metadata management, and highlight the importance of data lineage in data governance. This hands-on demonstration will equip participants with the knowledge to effectively navigate and utilize metastores in a cloud environment.
...
-- MAGIC    
-- MAGIC    - Observe the catalogs listed in the **Catalog** pane and select *Open Link in New Tab*.
-- MAGIC
-- MAGIC    - The items in this list resemble those from the `SHOW CATALOGS` SQL statement we executed earlier.
-- MAGIC
-- MAGIC    - Expand your unique catalog name, then expand **example**. This displays a list of tables, views, and functions.
-- MAGIC    
-- MAGIC    - Expand **tables**, then select **vw_gold** to see detailed information regarding the view. 
-- MAGIC    
-- MAGIC    - From here, you can view the schema, sample data, details, and permissions (which we'll get to shortly).
-- MAGIC
...
-- MAGIC
-- MAGIC 3. Select the **See lineage graph** button in the upper right corner.
-- MAGIC
