...
-- MAGIC %md
-- MAGIC 6. Run the query below to view the **customers** table. 
-- MAGIC
...
-- MAGIC - Selects all columns from the **customers_new** table.
-- MAGIC
-- MAGIC - Redact all values in the **c_phone** column to *REDACTED PHONE NUMBER* unless you are in the `is_account_group_member('admins')`
-- MAGIC     - HINT: Use a `CASE WHEN` statement in the `SELECT` clause.
-- MAGIC
-- MAGIC - Restrict the rows where **c_nationkey** is equal to *21* unless you are in the `is_account_group_member('admins')`.
